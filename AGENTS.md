...
- **Database Enums**: NEVER hardcode enum values (e.g., `'alpha'`, `'top_gun'`) in UI components or forms. Always import them from `Constants.public.Enums` exported by the generated database types (`src/types/database.types.ts`).
